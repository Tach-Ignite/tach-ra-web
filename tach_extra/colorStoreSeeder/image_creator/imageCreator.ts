...
function createCopyWithRandomColor(svg: string, randomColor: string): string {
  const strokeColor =
...
export function createCopiesWithRandomColor(
  svg: string,
  numCopies: number,
  folderName: string,
) {
  ...
  while (usedColors.size < numCopies) {
    const randomColor = generateRandomHexColor();
    if (!usedColors.has(randomColor)) {
      usedColors.add(randomColor);
      const svgImage = createCopyWithRandomColor(svg, randomColor);
      const fileName = `${folderName}/${randomColor}.svg`;
      fs.writeFileSync(fileName, svgImage);
      console.log(`File ${fileName} created.`);
    }
  }
  console.log(`${usedColors.size} images are created.`);
}
...
const numCopies = parseInt(process.argv[2], 10) || 5;
